...
class AjaxQuickCheckout extends \Opencart\System\Engine\Controller {
    ...
    public function save() {
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            //opencart module menu
            $new_post = array();
            $new_post['module_ajax_quick_checkout_status'] = isset($this->request->post[$this->codename.'_status']) ? (int)$this->request->post[$this->codename.'_status'] : 0;
            $this->model_setting_setting->editSetting('module_ajax_quick_checkout', $new_post);
    ...
    public function addSetting(){
        ...
        //opencart module menu
        $new_post = array();
        $new_post['module_ajax_quick_checkout_status'] = 1;
        $this->model_setting_setting->editSetting('module_ajax_quick_checkout', $new_post);
    ...
    public function editor(){
        ...
        $language = $this->config->get('config_language') ? $this->config->get('config_language') : $this->config->get('config_language_admin');
        $data['editor'] = $url.'index.php?route=checkout/checkout&edit=1&setting_id='.$setting_id . '&language=' . $language;
