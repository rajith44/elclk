<?php
namespace Opencart\Catalog\Model\Extension\CourierService\Shipping;

/**
 * Class CourierService
 *
 * Frontend quote provider for Courier Service.
 */
class CourierService extends \Opencart\System\Engine\Model {
	private const TABLE_NAME = DB_PREFIX . 'courier_service_rates';
	...
	public function getQuote(array $address): array {
		...
		if (!$this->config->get('shipping_courier_service_status')) {
			return [];
		}

		$method_title = $this->config->get('shipping_courier_service_title') ?: $this->language->get('text_title');
		$sort_order = (int)$this->config->get('shipping_courier_service_sort_order');
		$tax_class_id = (int)$this->config->get('shipping_courier_service_tax_class_id');
		...
		$default_weight = (float)$this->config->get('shipping_courier_service_default_weight');
		$additional_weight_step = (float)$this->config->get('shipping_courier_service_additional_weight');
		...
		$quote_data['courier_service'] = [
			'code' => 'courier_service.courier_service',
	...
	private function resolveCity(array $address): string {
		return trim((string)($address['zone'] ?? ''));
	}
